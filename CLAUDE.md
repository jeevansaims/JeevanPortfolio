...
2. **Academy** (`/academy/*`) - An educational platform for quantitative finance and mathematics with authentication, personalized learning roadmaps, and subscription features
...
- Middleware: `middleware.ts` protects `/academy/dashboard` routes and handles authentication redirects
...
**Protected Routes:**
- `/academy/dashboard/*` - requires authenticated Supabase user with verified email
- Middleware redirects unverified users to `/academy/verify-email`
- Middleware redirects logged-in users away from `/academy/login` and `/academy/register`

### Academy Feature Architecture

**Quiz → Roadmap Generation Flow:**
1. User takes quiz at `/academy/quiz` - answers stored in `roadmap_quiz_responses` table
...
**Key Files:**
- Quiz types: `app/academy/quiz/types/roadmap-quiz.tsx`
- Roadmap types: `app/academy/types/roadmap.ts`
- OpenAI prompt: `app/academy/lib/roadmap-prompt.ts`
- Generation API: `app/api/generate-roadmap/route.ts`

**Subscription System:**
- User profiles in `profiles` table track `membership_tier` ('free' or 'pro')
- Academy layout (`app/academy/layout.tsx`) checks subscription status and passes to sidebar
...
Located in `app/academy/actions/auth.ts`:
...
6. **Form Handling:**
   - Academy uses server actions with FormData
...
- Type definitions colocated with features (e.g., `app/academy/types/`)
