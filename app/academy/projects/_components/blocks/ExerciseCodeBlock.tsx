  ...
  const runTests = async () => {
    ...
    try {
      ...
      for (const testCase of block.testCases) {
        try {
          // Build test execution code in Python
          let testCode = `
import numpy as np
import pandas as pd
import json

# Helper to convert numpy/pandas to JSON-serializable
...
`;
          ...
          // Handle test inputs - they can be Python code strings or direct values
          if (Array.isArray(testCase.input)) {
            const args = testCase.input.map((inp, idx) => {
              if (typeof inp === 'string' && (inp.includes('np.') || inp.includes('pd.'))) {
                // It's Python code, evaluate it
                return `arg_${idx} = ${inp}`;
              } else {
                // It's a direct value
...
`;
          ...
          // Execute test in Python and parse JSON result
      ...
      // If all tests passed, trigger completion
      if (allPassed) {
  ...
  return (
    <>
        ...
        <div className="p-4 bg-green-500/10 border border-green-500/20 rounded-lg flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-zinc-400">
              Great job! Block marked as complete.
            </p>
          </div>
        </div>
      )}
      ...
      {/* Confirmation Dialog for revealing solution */}
